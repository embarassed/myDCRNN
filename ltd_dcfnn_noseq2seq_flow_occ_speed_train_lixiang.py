...
from model.ltd_dcfnn_noseq2seq_flow_occ_speed_supervisor import LTDDCFNNFOSSupervisor
...
flags.DEFINE_string('config_filename', 'data/model/dcfnn_config_lixiang.yaml', 'Configuration filename for restoring the model.')
...
def main():
    ...
    with open(FLAGS.config_filename) as f:
        ...
        with tf.Session(config=tf_config) as sess:
            supervisor = LTDDCFNNFOSSupervisor(traffic_reading_df=traffic_reading_df_s[0] ,traffic_reading_df_addition=traffic_reading_df_s[1:], adj_mx=adj_mx, config=supervisor_config)
