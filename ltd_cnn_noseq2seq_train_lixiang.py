...
from model.ltd_cnn_noseq2seq_supervisor import LTDCNNSupervisor
...
flags.DEFINE_string('config_filename', 'data/model/cnn_config_lixiang.yaml', 'Configuration filename for restoring the model.')
...
flags.DEFINE_integer('num_cnn_layers', '6', 'num_cnn_layers')
flags.DEFINE_integer('out_channels', '64', 'out_channels')
...
def main():
    ...
    with open(FLAGS.config_filename) as f:
        ...
        for name in ['batch_size', 'cl_decay_steps', 'epochs', 'horizon', 'learning_rate', 'l1_decay',
                     ...
                     'patience', 'seq_len', 'verbose', 'max_diffusion_step', 'num_cnn_layers', 'out_channels']:
        ...
        with tf.Session(config=tf_config) as sess:
            supervisor = LTDCNNSupervisor(traffic_reading_df=traffic_reading_df, adj_mx=adj_mx,
