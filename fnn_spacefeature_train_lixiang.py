...
from model.fnn_spacefeature_supervisor import FNNFeatureSupervisor
...
flags.DEFINE_string('config_filename', 'data/model/fnn_spacefeature_config_lixiang.yaml', 'Configuration filename for restoring the model.')
...
def main():
    ...
    with open(FLAGS.config_filename) as f:
        ...
        for name in ['batch_size', 'epochs', 'learning_rate', 'l1_decay',
        ...
        with tf.Session(config=tf_config) as sess:
            supervisor = FNNFeatureSupervisor(traffic_reading_df=traffic_reading_df, config=supervisor_config)
