...
from model.ltd_dcrnn_newgraph_stack_supervisor import LTDDCRNNSTACKSupervisor
...
flags.DEFINE_string('newgraph_pkl_filename', 'data/sensor_graph/weight_matrix_100.pkl',
                    'Pickle file containing: sensor_ids, sensor_id_to_ind_map, dist_matrix')
...
def main():
    ...
    with open(FLAGS.config_filename) as f:
        ...
        adj_mx_add = load_pickle(FLAGS.newgraph_pkl_filename)
        adj_mx[adj_mx < 0.1] = 0
        # for i in range(206):
        #     for j in range(206):
        #         adj_mx_add[i][j] = adj_mx_add[i][j]
        #         if adj_mx_add[i][j] < 0:
        #             adj_mx_add[i][j] = -adj_mx_add[i][j]
        #         if i == j:
        #             adj_mx_add[i][j] = 1
        ...
        for i in range(206):
            for j in range(206):
                if adj_mx[i][j] != 0 and i != j:
                    adj_mx[i][j] = adj_mx[i][j] * (1 + adj_mx_add[i][j])
        ...
        with tf.Session(config=tf_config) as sess:
            supervisor = LTDDCRNNSTACKSupervisor(traffic_reading_df=traffic_reading_df, adj_mx=adj_mx,
