...
from model.ltd_fnn_noseq2seq_flow_occ_speed_supervisor import LTDFNNFOSSupervisor
...
flags.DEFINE_string('config_filename', 'data/model/fnn_config_lixiang.yaml', 'Configuration filename for restoring the model.')
...
flags.DEFINE_string('data', 'flow,speed,occ', 'N.B. first must is flow! input data type: flow/speed/occ')
...
def main():
    ...
    with open(FLAGS.config_filename) as f:
        ...
        sensor_ids, sensor_id_to_ind, _ = load_graph_data(FLAGS.graph_pkl_filename)
        logger.info('Loading traffic data from Redis')
        traffic_reading_df_s = []
        data_s = FLAGS.data.split(',')
        for data in data_s:
            traffic_reading_df = generateDataframeFromRedis(data)
        ...
        if FLAGS.data:
            supervisor_config['data'] = FLAGS.data
        ...
        with tf.Session(config=tf_config) as sess:
            supervisor = LTDFNNFOSSupervisor(traffic_reading_df=traffic_reading_df_s[0] ,traffic_reading_df_addition=traffic_reading_df_s[1:], config=supervisor_config)
