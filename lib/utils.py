...
def ltd_generate_graph_seq2seq_io_data_with_time(df, batch_size, seq_len, horizon, num_nodes, scaler=None,
                                             add_time_in_day=True, add_day_in_week=False):
    """
    ltd: long term depends
    :param df:
    :param batch_size:
    :param seq_len:
    :param horizon:
    ...
    data = data[:1 * batch_len_0, :, :].reshape((1, batch_len_0, num_nodes, -1))
    input_dim = data.shape[-1]
    # epoch_size = batch_len - seq_len - horizon + 1
    epoch_size_0 = batch_len_0 - seq_len - horizon + 1
    x, y = [], []
    for i in range(epoch_size_0):
        x_i = data[:, i: i + seq_len, ...]
        y_i = data[:, i + seq_len: i + seq_len + horizon, :, :]
        x.append(x_i)
        y.append(y_i)
    x = np.stack(x, axis=0)  #(epoch_size_0, 1, seq_len, num_nodes, input_dim)
    y = np.stack(y, axis=0)  #(epoch_size_0, 1, horizon, num_nodes, input_dim)
    if batch_size != 1:
        epoch_size = epoch_size_0 * 1 // batch_size
        x = x[:epoch_size * batch_size, :, :, :]
        y = y[:epoch_size * batch_size, :, :, :]
        x = np.transpose(x, axes=[2, 3, 4, 0, 1])
        x = np.reshape(x, newshape=(seq_len, num_nodes, input_dim, -1))
        x = np.reshape(x, newshape=(seq_len, num_nodes, input_dim, epoch_size, batch_size))
        x = np.transpose(x, axes=[3, 4, 0, 1, 2])
        y = np.transpose(y, axes=[2, 3, 4, 0, 1])
        y = np.reshape(y, newshape=(horizon, num_nodes, input_dim, -1))
        y = np.reshape(y, newshape=(horizon, num_nodes, input_dim, epoch_size, batch_size))
        y = np.transpose(y, axes=[3, 4, 0, 1, 2])
    return x, y

def ltd_generate_graph_seq2seq_io_data_with_time_2(df, batch_size, seq_len, horizon, num_nodes, scaler=None,
                                             add_time_in_day=True, add_day_in_week=False):
    """
    ltd: long term depends
    :param df:
...
def ltd_train_val_test_split_df(df, seq_len, val_ratio=0.1, test_ratio=0.2):
    n_sample, _ = df.shape
