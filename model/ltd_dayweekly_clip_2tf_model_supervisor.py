...
class LTDTFModelSupervisor(object):
    ...
    def __init__(self, config, df_data, df_data_dayly, df_data_weekly, **kwargs):
        ...
        self._x_small_train, self._x_small_val, self._x_small_test, self._x_train, self._y_train, self._x_val, self._y_val, self._x_test, self._y_test = self._prepare_train_val_test_data_clip()
    ...
    def train(self, sess, **kwargs):
        ...
        while self._epoch <= epochs:
            ...
            train_results = TFModel.run_epoch(sess, self._train_model, inputs_small=self._x_small_train,
            ...
            val_results = TFModel.run_epoch(sess, self._val_model, inputs_small=self._x_small_val,
                                            inputs=self._x_val, inputs_dayly=self._dayly_x_val, inputs_weekly=self._weekly_x_val,
                                            labels=self._y_val,
                                            train_op=None)
    ...
    def test_and_write_result(self, sess, global_step, **kwargs):
        ...
        start_time = time.time()
        test_results = TFModel.run_epoch(sess, self._test_model, inputs_small=self._x_small_test,
                                         inputs=self._x_test, inputs_dayly=self._dayly_x_test, inputs_weekly=self._weekly_x_test,
    ...
    def _prepare_train_val_test_data_clip(self):
        ...
        x_small_train = x_train[:, :, int(seq_len / clip)*(-1) :, :, :]
        x_small_val = x_val[:, :, int(seq_len / clip)*(-1) :, :, :]
        x_small_test = x_test[:, :, int(seq_len / clip)*(-1) :, :, :]
        ...
        return x_small_train, x_small_val, x_small_test, x_train, y_train, x_val, y_val, x_test, y_test
