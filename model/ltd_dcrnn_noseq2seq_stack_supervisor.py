...
from model.dcrnn_noseq2seq_stack_model import DCRNNSTACKModel
from model.ltd_tf_model_supervisor import LTDTFModelSupervisor


class LTDDCRNNSTACKSupervisor(LTDTFModelSupervisor):
    """
    ...
    def __init__(self, traffic_reading_df, adj_mx, config):
        self._adj_mx = adj_mx
        super(LTDDCRNNSTACKSupervisor, self).__init__(config, df_data=traffic_reading_df)
    ...
    def _generate_run_id(config):
        ...
        if filter_type == 'random_walk':
            ...
            filter_type_abbr = 'DR'
        run_id = 'ltd_dcrnn_noseq2seq_stack_%s_%d_h_%d_%s_%s_lr_%g_bs_%d_d_%.2f_sl_%d_%s_%s/' % (
