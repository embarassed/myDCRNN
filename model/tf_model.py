...
class TFModel(object):
    ...
    def rnn_encoder_variant(self, inputs, batch_size, encoding_cells, initial_states=None):
        ...
        with tf.variable_scope("rnn_encoder") as rnn_vs:
            ...
            for i, inp in enumerate(inputs):
                ...
                for i, cell in enumerate(encoding_cells):
    ...
    def attention_rnn_decoder_variant(self, labels, num_nodes, num_heads, initial_states, decoding_cells, loop_function=None, attention_function=None):
        ...
        with tf.variable_scope("rnn_decoder") as rnn_vs:
            ...
            for i, inp in enumerate(labels):
                ...
                # att_out = tf.transpose(att_out, perm=[0, 1, 3, 2])  #[batch_size, num_nodes, -1, num_heads]
                # # conv1*1:
                # w1 = tf.get_variable('cnn1_w', [1, 1, num_heads, 12], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
                # att_out = tf.nn.conv2d(att_out, w1, strides=[1, 1, 1, 1], padding='VALID')
                # b1 = tf.get_variable('cnn1_b', [12], dtype=tf.float32, initializer=tf.constant_initializer(0.0, dtype=tf.float32))
                # att_out = tf.reshape(tf.nn.tanh(att_out + b1), shape=[batch_size * num_nodes, -1])
                inp = tf.concat([tf.reshape(inp, shape=[batch_size * num_nodes, -1]), self.linear([tf.reshape(state, shape=[batch_size * num_nodes, -1])] + attns, input_size)], axis=1)
                ...
                with tf.variable_scope("AttnOutputProjection"):
                    dec_outputs = tf.reshape(self.linear([tf.reshape(state, shape=[batch_size * num_nodes, -1])] + attns, output_size), shape=[batch_size, -1])
                outputs.append(dec_outputs)
                if loop_function is not None:
                    prev = dec_outputs
        return outputs, state

    def rnn_encoder(self, inputs, batch_size, encoding_cells, scope=None):
        # rnn_encoder:
        with tf.variable_scope(scope or 'rnn_encoder') as enc_rnn_vs:
    ...
    def rnn_decoder(self, labels, initial_states, decoding_cells, loop_function=None, scope=None):
        # rnn_decoder:
        with tf.variable_scope(scope or "rnn_decoder") as dec_rnn_vs:
