...
from model.cnn_noseq2seq_model import CNNModel
from model.ltd_tf_model_supervisor import LTDTFModelSupervisor


class LTDCNNSupervisor(LTDTFModelSupervisor):
    """
    ...
    def __init__(self, traffic_reading_df, adj_mx, config):
        self._adj_mx = adj_mx
        super(LTDCNNSupervisor, self).__init__(config, df_data=traffic_reading_df)
    ...
    def _build_train_val_test_models(self):
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('CNN', reuse=False):
                train_model = CNNModel(is_training=True, config=train_config, scaler=self._scaler,
                                         adj_mx=self._adj_mx)

        with tf.name_scope('Val'):
            with tf.variable_scope('CNN', reuse=True):
                val_model = CNNModel(is_training=False, config=train_config, scaler=self._scaler,
                                       adj_mx=self._adj_mx)

        with tf.name_scope('Test'):
            with tf.variable_scope('CNN', reuse=True):
                test_model = CNNModel(is_training=False, config=test_config, scaler=self._scaler,
                                        adj_mx=self._adj_mx)
    ...
    def _generate_run_id(config):
        ...
        num_cnn_layers = config.get('num_cnn_layers')
        out_channels = config.get('out_channels')
        seq_len = config.get('seq_len')
        structure = '-'.join(
            ['%d' % out_channels for _ in range(num_cnn_layers)])
        ...
        if filter_type == 'random_walk':
            ...
            filter_type_abbr = 'DR'
        run_id = 'ltd_cnn_noseq2seq_fsize_%d_%s_%d_h_%d_%s_lr_%g_bs_%d_d_%.2f_sl_%d_%s_%s/' % (
            filter_size, filter_type_abbr, max_diffusion_step, horizon,
