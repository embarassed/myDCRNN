...
class DCRNNSTACKModel(TFModel):
    def __init__(self, is_training, config, scaler=None, adj_mx=None):
        super(DCRNNSTACKModel, self).__init__(config, scaler=scaler)
        ...
        gcncell = GCNCell(gcn_units, adj_mx=adj_mx, max_diffusion_step=max_diffusion_step, num_nodes=num_nodes, filter_type=filter_type)
        ...
        cells = [cell] * (num_rnn_layers - 1) + [cell_with_projection]
        # encoding_cells = [cell] * num_rnn_layers
        # decoding_cells = [cell] * (num_rnn_layers - 1) + [cell_with_projection]

        global_step = tf.train.get_or_create_global_step()
        # Outputs: (batch_size, timesteps, num_nodes, output_dim)
        with tf.variable_scope('DCRNN_STACK_SEQ'):
            ...
            gcn_outputs = []
            for time, gcn_input_ in enumerate(inputs):
                with tf.variable_scope('gcn_%d' % time):
                    gcn_output_ = gcn_layers(gcn_input_)
                gcn_outputs.append(gcn_output_)
            GO_SYMBOL = tf.zeros(shape=(batch_size, num_nodes * (gcn_units)))
            for horizon_i in range(horizon):
                gcn_outputs.append(GO_SYMBOL)
            # construct second-layers: GRU(seq2seq)
            # rnn_encoder:
            h_s, _ = self.rnn_encoder(gcn_outputs, batch_size, cells)
            # rnn_decoder:
            # dec_outputs, _ = self.rnn_decoder(labels, enc_states, decoding_cells, loop_function)
        outputs = h_s[-horizon-1:]
