...
from model.dcmgu_cell import DCMGUCell
from model.mgu_cell import MGUCell
from model.tf_model import TFModel


class DCMGUModel(TFModel):
    def __init__(self, is_training, config, scaler=None, adj_mx=None):
        super(DCMGUModel, self).__init__(config, scaler=scaler)
        ...
        if max_diffusion_step == 0:
            cell = MGUCell(rnn_units, num_nodes=num_nodes)
            cell_with_projection = MGUCell(rnn_units, num_nodes=num_nodes, num_proj=output_dim)
        else:
            cell = DCMGUCell(rnn_units, adj_mx, max_diffusion_step=max_diffusion_step, num_nodes=num_nodes, filter_type=filter_type)
            cell_with_projection = DCMGUCell(rnn_units, adj_mx, max_diffusion_step=max_diffusion_step, num_nodes=num_nodes,
        ...
        with tf.variable_scope('DCMRU_SEQ'):
