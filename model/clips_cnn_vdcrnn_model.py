...
class CLIPSCNNDCRNNModel(TFModel):
    def __init__(self, is_training, config, scaler=None, adj_mx=None):
        ...
        with tf.variable_scope('DCRNN_SEQ'):
            ...
            # # feed in No.3 CNN:
            # W3 = tf.get_variable('cnn3_w', [1, 2, 64, 32], dtype=dtype,
            #                      initializer=tf.contrib.layers.xavier_initializer())
            # inputs = tf.nn.conv2d(inputs, W3, strides=[1, 1, 1, 1], padding='VALID')
            # b3 = tf.get_variable('cnn3_b', [32], dtype=dtype, initializer=tf.constant_initializer(0.0, dtype=dtype))
            # inputs = tf.nn.relu(inputs + b3)
            # feed in No.4 CNN:
            # W4 = tf.get_variable('cnn4_w', [1, 2, 16, 16], dtype=dtype,
            #                      initializer=tf.contrib.layers.xavier_initializer())
            # inputs = tf.nn.conv2d(inputs, W4, strides=[1, 1, 1, 1], padding='VALID')
            # b4 = tf.get_variable('cnn4_b', [16], dtype=dtype, initializer=tf.constant_initializer(0.0, dtype=dtype))
            # inputs = tf.nn.relu(inputs + b4)
