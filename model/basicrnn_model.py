...
from model.basicrnn_cell import BasicRNNCell
from model.tf_model import TFModel


class BasicRNNModel(TFModel):
    def __init__(self, is_training, config, scaler=None, adj_mx=None):
        super(BasicRNNModel, self).__init__(config, scaler=scaler)
        ...
        cell = BasicRNNCell(rnn_units, num_nodes=num_nodes)
        cell_with_projection = BasicRNNCell(rnn_units, num_nodes=num_nodes, num_proj=output_dim)
        ...
        with tf.variable_scope('BasicRNN_SEQ'):
