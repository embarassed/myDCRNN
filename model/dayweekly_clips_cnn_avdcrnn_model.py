            ...
            with tf.variable_scope("dayweeklycnn_rnn_decoder") as cnnrnn_vs:
                ...
                for i, inp in enumerate(labels):
                    ...
                    if i < len(labels) - 1:
                        ...
                        attns = attention([tf.reshape(state, shape=[batch_size * num_nodes, -1])])
                        ...
                        inp = tf.concat([inp, tf.reshape(att_out * state, shape=[batch_size, num_nodes, -1]), dayly_inp], axis=-1)  # 130d
                        # inp = dayly_inp   # 128d
                        inp = tf.reshape(inp, shape=[batch_size, -1])
                        # # (2) weekly cnn1:
