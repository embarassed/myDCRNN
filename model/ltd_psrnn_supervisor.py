...
from model.psrnn_model import PSRNNModel
from model.ltd_psrnn_tf_model_supervisor import LTDTFModelSupervisor


class LTDPSRNNSupervisor(LTDTFModelSupervisor):
    """
    Do experiments using Graph Random Walk RNN model.
    """

    def __init__(self, traffic_reading_df, config):
        super(LTDPSRNNSupervisor, self).__init__(config, df_data=traffic_reading_df)
    ...
    def _build_train_val_test_models(self):
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('PSRNN', reuse=False):
                train_model = PSRNNModel(is_training=True, config=train_config, scaler=self._scaler, params=self._params)

        with tf.name_scope('Val'):
            with tf.variable_scope('PSRNN', reuse=True):
                val_model = PSRNNModel(is_training=False, config=train_config, scaler=self._scaler, params=self._params)

        with tf.name_scope('Test'):
            with tf.variable_scope('PSRNN', reuse=True):
                test_model = PSRNNModel(is_training=False, config=test_config, scaler=self._scaler, params=self._params)
    ...
    def _generate_run_id(config):
        ...
        if filter_type == 'random_walk':
            ...
            filter_type_abbr = 'DR'
        run_id = 'ltd_psrnn_%s_%d_h_%d_%s_lr_%g_bs_%d_d_%.2f_sl_%d_%s_%s/' % (
