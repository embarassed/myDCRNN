...
from model.lstm_noseq2seq_model import LSTMModel
from model.ltd_tf_model_supervisor import LTDTFModelSupervisor


class LTDLSTMSupervisor(LTDTFModelSupervisor):
    """
    ...
    def __init__(self, traffic_reading_df, adj_mx, config):
        self._adj_mx = adj_mx
        super(LTDLSTMSupervisor, self).__init__(config, df_data=traffic_reading_df)
    ...
    def _build_train_val_test_models(self):
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('DCRNN', reuse=False):
                train_model = LSTMModel(is_training=True, config=train_config, scaler=self._scaler,
                                         adj_mx=self._adj_mx)

        with tf.name_scope('Val'):
            with tf.variable_scope('DCRNN', reuse=True):
                val_model = LSTMModel(is_training=False, config=train_config, scaler=self._scaler,
                                       adj_mx=self._adj_mx)

        with tf.name_scope('Test'):
            with tf.variable_scope('DCRNN', reuse=True):
                test_model = LSTMModel(is_training=False, config=test_config, scaler=self._scaler,
                                        adj_mx=self._adj_mx)
    ...
    def _generate_run_id(config):
        ...
        if filter_type == 'random_walk':
            ...
            filter_type_abbr = 'DR'
        run_id = 'ltd_lstm_noseq2seq_h_%d_%s_lr_%g_bs_%d_d_%.2f_sl_%d_%s_%s/' % (
            horizon,
