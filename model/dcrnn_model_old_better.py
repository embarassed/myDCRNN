...
class DCRNNModel(TFModel):
    def __init__(self, is_training, config, scaler=None, adj_mx=None):
        ...
        encoding_cells = DCGRUCell(rnn_units, adj_mx, max_diffusion_step=max_diffusion_step, num_nodes=num_nodes, filter_type=filter_type)
        decoding_cells = DCGRUCell(rnn_units, adj_mx, max_diffusion_step=max_diffusion_step, num_nodes=num_nodes, filter_type=filter_type)
        decoding_cells_with_projection = DCGRUCell(rnn_units, adj_mx, max_diffusion_step=max_diffusion_step, num_nodes=num_nodes,
            ...
            for cell_layer in range(num_rnn_layers):
                with tf.variable_scope('rnn_encoder_%d' % cell_layer):
                    if cell_layer == 0:
                        ...
                        enc_outputs, state = tf.contrib.rnn.static_rnn(encoding_cells, enc_outputs, initial_state=state, dtype=tf.float32)
            # rnn_decoder:
                ...
                for i, inp in enumerate(labels):
                    ...
                    for cell_layer in range(num_rnn_layers-1):
                        with tf.variable_scope('cell_%d' % cell_layer) as cellvs:
                            ...
                            if cell_layer == 0:
                                dec_outputs, state = decoding_cells(inp, state)
                            else:
