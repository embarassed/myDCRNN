...
from model.dayweekly_clips_cnn_avdcrnn_model import CLIPSCNNDCRNNModel
from model.ltd_dayweekly_clip_tf_model_supervisor import LTDTFModelSupervisor


class LTDDCRNNSupervisor(LTDTFModelSupervisor):
    """
    Do experiments using Graph Random Walk RNN model.
    LTDDCRNNSupervisor(traffic_reading_df=traffic_reading_df_s[0],
    ...
    def __init__(self, traffic_reading_df, traffic_reading_df_dayly, traffic_reading_df_weekly, adj_mx, config):
        self._adj_mx = adj_mx
        super(LTDDCRNNSupervisor, self).__init__(config, df_data=traffic_reading_df, df_data_dayly=traffic_reading_df_dayly, df_data_weekly=traffic_reading_df_weekly)
    ...
    def _build_train_val_test_models(self):
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('DCRNN', reuse=False):
                train_model = CLIPSCNNDCRNNModel(is_training=True, config=train_config, scaler=self._scaler,
                                         adj_mx=self._adj_mx)

        with tf.name_scope('Val'):
            with tf.variable_scope('DCRNN', reuse=True):
                val_model = CLIPSCNNDCRNNModel(is_training=False, config=train_config, scaler=self._scaler,
                                       adj_mx=self._adj_mx)

        with tf.name_scope('Test'):
            with tf.variable_scope('DCRNN', reuse=True):
                test_model = CLIPSCNNDCRNNModel(is_training=False, config=test_config, scaler=self._scaler,
                                        adj_mx=self._adj_mx)
    ...
    def _generate_run_id(config):
        ...
        if filter_type == 'random_walk':
            ...
            filter_type_abbr = 'DR'
        run_id = 'ltd_day12week4ly_clip_%d_rseql24_cnn3_64D_attention_h2_vdcrnn_%s_%d_h_%d_%s_lr_%g_bs_%d_d_%.2f_sl_%d_%s_%s/' % (
            clip, filter_type_abbr, max_diffusion_step, horizon,
