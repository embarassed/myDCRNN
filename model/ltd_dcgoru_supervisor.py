...
from model.dcgoru_model import DCGORUModel
from model.ltd_tf_model_supervisor import LTDTFModelSupervisor


class LTDDCGORUSupervisor(LTDTFModelSupervisor):
    """
    ...
    def __init__(self, traffic_reading_df, adj_mx, config):
        self._adj_mx = adj_mx
        super(LTDDCGORUSupervisor, self).__init__(config, df_data=traffic_reading_df)
    ...
    def _build_train_val_test_models(self):
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('DCGORU', reuse=False):
                train_model = DCGORUModel(is_training=True, config=train_config, scaler=self._scaler,
                                         adj_mx=self._adj_mx)

        with tf.name_scope('Val'):
            with tf.variable_scope('DCGORU', reuse=True):
                val_model = DCGORUModel(is_training=False, config=train_config, scaler=self._scaler,
                                       adj_mx=self._adj_mx)

        with tf.name_scope('Test'):
            with tf.variable_scope('DCGORU', reuse=True):
                test_model = DCGORUModel(is_training=False, config=test_config, scaler=self._scaler,
                                        adj_mx=self._adj_mx)
    ...
    def _generate_run_id(config):
        ...
        if filter_type == 'random_walk':
            ...
            filter_type_abbr = 'DR'
        run_id = 'ltd_dcgoru_%s_%d_h_%d_%s_lr_%g_bs_%d_d_%.2f_sl_%d_%s_%s/' % (
