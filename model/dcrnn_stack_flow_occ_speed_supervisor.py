...
class DCRNNSTACKFOSSupervisor(TFModelFOSSupervisor):
    """
    ...
    def __init__(self, traffic_reading_df, traffic_reading_df_addition, adj_mx, config):
        self._adj_mx = adj_mx
        super(DCRNNSTACKFOSSupervisor, self).__init__(config, df_data=traffic_reading_df, df_addition_data_s=traffic_reading_df_addition)
    ...
    def _generate_run_id(config):
        ...
        if filter_type == 'random_walk':
            ...
            filter_type_abbr = 'DR'
        run_id = 'dcrnn_stack_%s_%s_%d_h_%d_%s_%s_lr_%g_bs_%d_d_%.2f_sl_%d_%s_%s/' % (
