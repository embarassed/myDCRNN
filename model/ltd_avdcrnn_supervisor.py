...
from model.avdcrnn_model import ADCRNNModel
...
class LTDDCRNNSupervisor(LTDTFModelSupervisor):
    ...
    def _build_train_val_test_models(self):
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('DCRNN', reuse=False):
                train_model = ADCRNNModel(is_training=True, config=train_config, scaler=self._scaler,
                                         adj_mx=self._adj_mx)

        with tf.name_scope('Val'):
            with tf.variable_scope('DCRNN', reuse=True):
                val_model = ADCRNNModel(is_training=False, config=train_config, scaler=self._scaler,
                                       adj_mx=self._adj_mx)

        with tf.name_scope('Test'):
            with tf.variable_scope('DCRNN', reuse=True):
                test_model = ADCRNNModel(is_training=False, config=test_config, scaler=self._scaler,
                                        adj_mx=self._adj_mx)
    ...
    def _generate_run_id(config):
        ...
        if filter_type == 'random_walk':
            ...
            filter_type_abbr = 'DR'
        run_id = 'ltd_attention_h%d_vdcrnn_%s_%d_h_%d_%s_lr_%g_bs_%d_d_%.2f_sl_%d_%s_%s/' % (
            num_heads, filter_type_abbr, max_diffusion_step, horizon,
