...
class GCNCell(base_layer.Layer):
    """
    自定义多节点的一个时间戳上的GCN
    """
    ...
    def __call__(self, inputs, state=None, scope=None):
        """GCN with Graph Convolution.
        :param inputs: (B, num_nodes * input_dim)

        :return
        - Output: A `2-D` tensor with shape `[batch_size x self.output_size]`.
        """
        with tf.variable_scope(scope or "gcn_cell"):
            with tf.variable_scope("relu"):
                output = self._activation(
                    self._gconv(inputs, self._num_units, bias_start=0.0, scope=scope))
        return output
