...
from model.fnn_spacefeature_model import FNNFeatureModel
from model.tf_model_spacefeature_supervisor import TFFeatureModelSupervisor


class FNNFeatureSupervisor(TFFeatureModelSupervisor):
    """
    Do experiments using Graph Random Walk RNN model.
    """

    def __init__(self, traffic_reading_df, config):
        super(FNNFeatureSupervisor, self).__init__(config, df_data=traffic_reading_df)
    ...
    def _build_train_val_test_models(self):
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('FNN', reuse=False):
                train_model = FNNFeatureModel(is_training=True, config=train_config, scaler=self._scaler)

        with tf.name_scope('Val'):
            with tf.variable_scope('FNN', reuse=True):
                val_model = FNNFeatureModel(is_training=False, config=train_config, scaler=self._scaler)

        with tf.name_scope('Test'):
            with tf.variable_scope('FNN', reuse=True):
                test_model = FNNFeatureModel(is_training=False, config=test_config, scaler=self._scaler)
    ...
    def _generate_run_id(config):
        ...
        run_id = 'fnn_feature_h_%s_lr_%g_bs_%d_d_%.2f_sl_%s_%s/' % (
