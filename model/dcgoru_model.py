...
from model.dcgoru_cell import DCGORUCell
from model.goru_cell import GORUCell
from model.tf_model import TFModel


class DCGORUModel(TFModel):
    def __init__(self, is_training, config, scaler=None, adj_mx=None):
        super(DCGORUModel, self).__init__(config, scaler=scaler)
        ...
        if max_diffusion_step == 0:
            for i in range(num_rnn_layers):
                encoding_cells.append(GORUCell(rnn_units, num_nodes=num_nodes, scope=("enc_%d" % i)))
                if i == num_rnn_layers - 1:
                    decoding_cells.append(
                        GORUCell(rnn_units, num_nodes=num_nodes, num_proj=output_dim, scope=("dec_%d" % i)))
                else:
                    decoding_cells.append(GORUCell(rnn_units, num_nodes=num_nodes, scope=("dec_%d" % i)))
        else:
            for i in range(num_rnn_layers):
                encoding_cells.append(
                    DCGORUCell(rnn_units, adj_mx, max_diffusion_step=max_diffusion_step, num_nodes=num_nodes,
                               filter_type=filter_type, scope=("enc_%d" % i)))
                if i == num_rnn_layers - 1:
                    decoding_cells.append(
                        DCGORUCell(rnn_units, adj_mx, max_diffusion_step=max_diffusion_step, num_nodes=num_nodes,
                                   filter_type=filter_type, num_proj=output_dim, scope=("dec_%d" % i)))
                else:
                    decoding_cells.append(
                        DCGORUCell(rnn_units, adj_mx, max_diffusion_step=max_diffusion_step, num_nodes=num_nodes,
                                   filter_type=filter_type, scope=("dec_%d" % i)))

        GO_SYMBOL = tf.zeros(shape=(batch_size, num_nodes * (output_dim + aux_dim)))

        global_step = tf.train.get_or_create_global_step()
        # Outputs: (batch_size, timesteps, num_nodes, output_dim)
        with tf.variable_scope('DCGORU_SEQ'):
